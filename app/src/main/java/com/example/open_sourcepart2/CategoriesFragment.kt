...
class CategoriesFragment : Fragment() {
    ...
    private fun showAddEditCategoryDialog(category: Category?) {
        ...
        if (category != null) {
            ...
            selectedColor = category.color ?: "#B22222"
        }
            ...
            try {
                ...
                lifecycleScope.launch {
                    if (category == null) {
                        ...
                        val id = database.categoryDao().insertCategory(newCategory)
                        if (id > 0) {
                        ...
                        val result = database.categoryDao().updateCategory(updatedCategory)
                        if (result > 0) {
    ...
    private fun deleteCategory(category: Category) {
        AlertDialog.Builder(requireContext())
            ...
            .setPositiveButton("Delete") { _, _ ->
                lifecycleScope.launch {
                    val result = database.categoryDao().deleteCategory(category)
                    if (result > 0) {
