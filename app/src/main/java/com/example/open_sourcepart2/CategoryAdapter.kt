...
class CategoryAdapter(
    ...
    inner class CategoryViewHolder(private val binding: ItemCategoryBinding) :
        ...
        fun bind(category: Category) {
            ...
            val initial = if (category.name.length >= 2) {
                category.name.take(2).uppercase()
            } else {
                category.name.uppercase()
            }
            binding.tvCategoryInitial.text = initial

            // Set background color for the initial circle
            try {
                binding.tvCategoryInitial.setBackgroundColor(Color.parseColor(category.color ?: "#7B1FA2"))
            } catch (e: Exception) {
                binding.tvCategoryInitial.setBackgroundColor(Color.parseColor("#7B1FA2"))
            }
