...
class InsightsFragment : Fragment() {
    ...
    private fun loadInsights() {
        ...
        lifecycleScope.launch {
            ...
            val monthlyExpenses = database.expenseDao().getTotalExpensesByDateRange(user.id, monthStart, monthEnd) ?: 0.0
            val monthlyIncome = database.incomeDao().getTotalIncome(user.id) ?: 0.0

            // Get budget
            val monthlyBudget = database.budgetDao().getBudgetByPeriod(user.id, "monthly")?.amount ?: 0.0
